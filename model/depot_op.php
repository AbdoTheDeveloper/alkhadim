...
class depot_op extends table
{
	...
	public function get_details($id)
	{
		...
		ddo.*, 
		d1.nom AS depot_src, 
		d2.nom AS depot_dest, 
		p.designation 
	  FROM 
		detail_depot_op ddo 
		LEFT JOIN depot d1 ON ddo.id_depot_src = d1.id 
		LEFT JOIN depot d2 ON ddo.id_depot_dest = d2.id 
		LEFT JOIN produit p ON ddo.id_produit = p.id_produit 
	  WHERE 
		ddo.id_depot_op = " . $id . " 
	  order by 
		ddo.id_detail desc";
