...
<body style="width:950px;margin:auto;">
  ...
  <?php if (isset($_POST['submit'])) {
    ...
    $query = "SELECT id_detail, v.id_vente, produit.id_produit, produit.designation, produit.code_bar, prix_produit, 
    SUM(detail_vente.qte_vendu) / COUNT(detail_vente.id_produit) AS vendus, 
    SUM(tv.qte_vendu) AS qte_vendu_prod, 
    SUM(detail_vente.qte_restante) / COUNT(detail_vente.id_produit) AS restantes , 
    SUM( detail_vente.prix_produit * detail_vente.qte_vendu ) as total_mt, SUM( detail_vente.prix_produit * qte_restante ) as total_restant FROM detail_vente 
    LEFT JOIN produit ON detail_vente.id_produit = produit.id_produit 
    LEFT JOIN tracking_vente tv ON produit.code_bar = tv.code_bar 
    WHERE detail_vente.id_vente IN (" . $ventes_string . ") 
          AND tv.date_vente BETWEEN '$date_d' AND '$date_f' " . $extra_filters1 . " 
    GROUP BY 
      id_produit";
    ...
    padding: 6px; color:black;"> Etat des ventes GLOBAL </span>
      ...
      ?>
      <h3 align="center" style="border: 2px solid black;
    padding: 6px; color:black;"> Etat des ventes du <span class="date">
          <?php echo $_POST['dd']; ?>
        </span> A <span class="date">
          <?php echo $_POST['df']; ?>
        </span>
      </h3>
      <?php
    ...
    padding: 6px; color:black;"><span class="date">
    ...
    <table class="datatables" border=1 style="border-style:none;">
      <tr class="row">
        ...
        <th scope="col">Qte Restants</th>
        <th scope="col">MT Restants</th>
        <!-- <th scope="col">Qte Vendu Par Client </th> -->
      </tr>
      ...
      foreach ($details as $det) {
        ...
        <tr>
          ...
          <td align="right">
            <?php echo number_format($det->restantes, 2, '.', '.') ?>
          </td>
          <td align="right">
            <?php echo number_format($det->total_restant, 2, '.', '.') ?>
          </td>
