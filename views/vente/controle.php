        ...
        <tr>
          ...
          <td class="nowrap">
            <?php if ($ligne->numbon != 0) { ?>
              <a target="_blank" href="<?php echo BASE_URL . "views/vente/facturebon.php?id=" . $ligne->id_vente; ?>&h=15" class="badge badge-primary">
          ...
          <td class="nowrap">
            <?php if (auth::user()['privilege'] == 'Admin') { ?>
              ...
              <!-- <a class="badge badge-success mb-2  " style="color: white;cursor: pointer;" title="Imprimmer Avec Prix"
                href="<?php // echo  BASE_URL . "views/vente/facturebon_avec_prix.php?id=" . $ligne->id_vente; 
                      ?>&h=15" target="_black">
                <i class=" simple-icon-printer" style="font-size: 15px;"></i>
              </a> -->
...
if ($_POST['act'] == 'getProd') {
  $pd = connexion::getConnexion()->query("SELECT * FROM produit WHERE id_produit = " . $_POST['id_prod'])->fetch(PDO::FETCH_OBJ);
  echo json_encode($pd);
  exit;
}
