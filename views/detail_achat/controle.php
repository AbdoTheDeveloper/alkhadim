...
//-----------------------------------------------------------------------------------Valider Les Achats ------------------------------------------------------------------------------------------------------
 ...
 elseif ($_POST['act'] == 'valide_detail_achat') {
	...
	// $statut = connexion::getConnexion()->exec('UPDATE detail_achat SET valide = 1 , date_validation = CURDATE() WHERE id_achat =' . $_POST['id']);
	$statut = connexion::getConnexion()->exec('UPDATE detail_achat SET valide = 1, date_validation = CURDATE() WHERE id_detail ='.$_POST['id']);
	...
	$result2 = connexion::getConnexion()->query("select da.valide,da.id_produit, da.id_depot, da.qte_achete as qte_achete from detail_achat da WHERE da.id_detail =" . $_POST['id']);
