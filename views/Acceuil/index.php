...
$produit = new produit();
$alertproduitc = count($produit->selectAlertQte());
...
$ca = connexion::getConnexion()->query("SELECT t1.montantv from 
    ...
    inner join detail_vente dv on dv.id_vente=v.id_vente 
    inner join produit p on dv.id_produit=p.id_produit
    where v.numbon <> 0 AND MONTH(v.date_vente) = MONTH(now()) AND YEAR(v.date_vente) = Year(now())
    order by id_vente desc  ) as t1 
    left join (select id_vente,ifnull(sum(montant),0) as avance 
...
<div class="container-fluid disable-text-selection">
      ...
      <div class="card">
        <div class="card-body ctm-card">
          ...
          <div>
            ...
            <span class="ctm-muted-text">Produits</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <?php
  $produit = new produit();
  $alertproduit = $produit->selectAlertQte();
