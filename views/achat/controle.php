				...
				<tr>
					...
					<td class="nowrap">
						<?php if ((int) auth::user()['achat'] == 1 || auth::user()['privilege'] == 'Admin') { ?>
							...
							<a class="badge badge-primary mb-2  " style="color: white;cursor: pointer;" title="Imprimmer Bon Reception " href="<?php echo BASE_URL . "views/achat/bon_reception.php?id=" . $ligne->id_achat; ?>&h=15" target="_black">
								<i class=" simple-icon-printer" style="font-size: 15px;"></i>
							</a>
						<?php } ?>
                      ...
                      <?php //endif; 
						...
						<!-- <?php // if ($ligne->valide == 0): 
								?> -->
						<a class="badge  mb-2 <?php echo !$ligne->valide ? 'badge-success url notlink ' : 'badge-secondary url notlink ' ?>" style="color: white;cursor: pointer;" data-url="detail_achat/index.php?id=<?php echo $ligne->id_achat; ?>&valide=true" title="<?php echo $ligne->valide ? 'commande validé ' : 'valider commande' ?>">
							<i class="simple-icon-check" style="font-size: 15px;"></i>
						</a>
						<?php // endif; 
						?>
...
if ($_POST['act'] == 'rech') {
	...
	$detail_query = "SELECT id_detail FROM `detail_achat` WHERE id_achat = " . $id_achat;


	$result = connexion::getConnexion()->query($detail_query)->fetchAll();

	foreach ($result as $row) {

		$_POST['id'] = $row['id_detail'];
	...
	$statut = connexion::getConnexion()->exec("INSERT INTO historique_modifs_achat(id_achat, date_modification, id_user) VALUES ($id, CURRENT_DATE, $id_user);");
...
if ($_POST['act'] == 'get_produit_achtee') {
	$depot = new depot();
	$res_depot = $depot->selectAll();
	foreach ($res_depot as $rep_depot) {
	?>
...
if ($_POST['act'] ==  "getAcheteQte") {
	$data  = connexion::getConnexion()->query("select da.qte_achete as qte , da.id_detail from detail_achat da , achat a where da.id_achat =  a.id_achat and da.id_produit = 
	" . $_POST['id_produit'] . " and a.id_achat =" . $_POST['id_achat'])->fetchAll(PDO::FETCH_OBJ);
