...
$data2 = connexion::getConnexion()->query("select p.code_bar,p.tva,p.unite,p.designation,p.poid,dv.prix_produit,p.poid,dv.qte_achete,dv.id_achat,dv.remise, dv.date_validation from detail_achat dv left join produit p on p.id_produit=dv.id_produit  where dv.id_achat in (" . trim($_GET["id"], ',') . ")");
...
// $pdf->Cell(10);
$pdf->Cell(10,5,"".$data1["raison_sociale"] ,0,1,'L',0);
...
$pdf->Cell(80,10, 'Bon de Reception N° : BC.' . $_GET["id"] . ' - ' . date("Y", strtotime($data1["date_achat"])),0,0,'L',0);
...
$pdf->Cell(30,10,'Date Validation',1,1,'C',0);
$pdf->SetFont('dejavusans', '', 9, '', true);
$total=$totalht=0;
$cont=0;
$pdf->SetFont('dejavusans', '', 9, '', true);
...
$pdf->Cell(30,$height,$ligne["date_validation"],'RB',1,'R',0);
